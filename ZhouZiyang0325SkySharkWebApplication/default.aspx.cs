...
namespace ZhouZiyang0325SkySharkWebApplication
{
    public partial class _default : System.Web.UI.Page
    {
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (Page.IsValid)
            {
                ...
                {
                    if (dsl.Tables["dtUsers"].Rows[0][1].ToString().Trim()==txtPassword.Text.Trim())
                    {
                        String Role;
                        Role = dsl.Tables["dtUsers"].Rows[0][2].ToString().Trim();
